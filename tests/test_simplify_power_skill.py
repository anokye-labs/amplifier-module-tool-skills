"""Tests for the /simplify power skill in amplifier-bundle-skills."""
...
# This file is at: amplifier-module-tool-skills/tests/test_simplify_power_skill.py
# The bundle is at: amplifier-bundle-skills/
BUNDLE_SKILLS_DIR = (
    Path(__file__).parent.parent.parent / "amplifier-bundle-skills" / "skills"
)

SIMPLIFY_SKILL_PATH = BUNDLE_SKILLS_DIR / "simplify" / "SKILL.md"


def test_simplify_skill_file_exists():
    """SKILL.md file must exist at amplifier-bundle-skills/skills/simplify/SKILL.md."""
    assert SIMPLIFY_SKILL_PATH.exists(), f"SKILL.md not found at {SIMPLIFY_SKILL_PATH}"


def test_simplify_skill_is_discoverable():
    """Skill must be discoverable via discover_skills()."""
    skills = discover_skills(BUNDLE_SKILLS_DIR)
    assert "simplify" in skills, (
        f"'simplify' skill not found via discover_skills(). Found: {list(skills.keys())}"
    )


def test_simplify_skill_context_is_fork():
...
def test_simplify_skill_disable_model_invocation_is_true():
...
def test_simplify_skill_model_role_is_critique():
...
def test_simplify_skill_user_invocable_is_true():
...
def test_simplify_skill_body_contains_arguments_placeholder():
    """Body must contain $ARGUMENTS placeholder for optional focus area."""
    body = extract_skill_body(SIMPLIFY_SKILL_PATH)
...
def test_simplify_skill_description_mentions_simplification():
...
def test_simplify_skill_body_describes_parallel_agents():
...
def test_simplify_skill_body_mentions_git_diff():
